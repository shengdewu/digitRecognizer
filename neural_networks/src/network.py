...
class Network(object):
    ...
    def update_mini_batch(self, mini_batch, eta):
        ...
        for x, y in mini_batch:
            ...
            delta_nabla_w, delta_nabla_b = self.__back_propagation__(y, x)
    ...
    def backpropagation(self, y, x):
        ...
        delta = self.cost_derivative(actives[-1], y) * self.sigmod_derivative(zs[-1])
        delta_w[-1] = np.dot(delta, actives[-2].transpose())
        delta_b[-1] = delta
        #backforward
        for lay in xrange(2, self.num_layers):
            delta = np.dot(self.weights[-lay + 1].transpose(), delta) * self.sigmod_derivative(zs[-lay])
    ...
    def __update_mini_batch__(self, mini_batch, alpha):
        ...
        self.weights = [w - alpha * sw / len(mini_batch) for w, sw in zip(self.weights ,sum_delta_w)]
        self.biases = [b - alpha * sb / len(mini_batch) for b, sb in zip(self.biases ,sum_delta_b)]
    ...
    def __back_propagation__(self, y, x):
        ...
        for lay in range(2, self.num_layers):
            delta = np.dot(self.weights[-lay + 1].transpose(), delta) * self.__sigmod_derivative__(zs[-lay])
            delta_w[-lay] = np.dot(delta, actives[-lay-1].transpose())
            delta_b[-lay] = delta
